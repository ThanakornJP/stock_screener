...
def model(tickers):
    ...
    for ticker in tickers:
        ...
        if len(dividends) < 1:        
            ...
            dividend_table['streak_end_date'].append('')     
            dividend_table['lastest_amount'].append(0.0)
            dividend_table['streak_start_date'].append('') 
            dividend_table['num_streak'].append(0)
            dividend_table['incremental_percentage'].append(0.0)
            dividend_table['payout_period'].append(0)
            dividend_table['number_survive_proof'].append(0)
            dividend_table['latest_survive_streak'].append('')

            #survive_years = int(datetime.now().year) - int(datetime.strptime(dividends[-1]['exOrEffDate'],  "%m/%d/%Y").year)
            dividend_table['number_survive_years'].append(0) 
        else:
